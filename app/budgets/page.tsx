  ...
  return (
    ...
    >
      ...
      <main className="max-w-[1600px] mx-auto px-6 py-8 sm:py-12 space-y-12 pb-32 transition-all duration-500 relative z-10">
        <div className="flex justify-between items-end">
          <div>
            ...
            <p className="text-[8px] sm:text-sm font-bold opacity-50 uppercase tracking-widest">
              Kontrol Pengeluaran & Tabungan
            </p>
          </div>
        </div>

        <div
          id="tour-budgets-summary"
          className="grid grid-cols-2 sm:grid-cols-3 gap-4 sm:gap-6"
        >
          <div
            className={`p-4 sm:p-6 rounded-[1.5rem] sm:rounded-[2rem] border transition-all ${isDark ? "glass-dark border-green-900/20" : "bg-green-50/50 border-green-100"}`}
          >
            <div className="flex justify-between items-start mb-2 sm:mb-4">
              <p className="text-[8px] sm:text-[10px] font-black text-green-500 uppercase tracking-[0.2em]">
                Pemasukan
              </p>
              <ArrowUpRight className="text-green-500 opacity-30" size={16} />
            </div>
            <p className="text-lg sm:text-2xl font-black text-green-600 tracking-tighter truncate">
              Rp {totalIncome.toLocaleString("id-ID")}
            </p>
          </div>
          <div
            className={`p-4 sm:p-6 rounded-[1.5rem] sm:rounded-[2rem] border transition-all ${isDark ? "glass-dark border-red-900/20" : "bg-red-50/50 border-red-100"}`}
          >
            <div className="flex justify-between items-start mb-2 sm:mb-4">
              <p className="text-[8px] sm:text-[10px] font-black text-red-500 uppercase tracking-[0.2em]">
                Pengeluaran
              </p>
              <ArrowDownRight className="text-red-500 opacity-30" size={16} />
            </div>
            <p className="text-lg sm:text-2xl font-black text-red-600 tracking-tighter truncate">
              Rp {totalOutcome.toLocaleString("id-ID")}
            </p>
          </div>
          <div
            className={`p-4 sm:p-6 rounded-[1.5rem] sm:rounded-[2rem] border transition-all col-span-2 sm:col-span-1 ${isDark ? "glass-dark border-indigo-900/20" : "bg-indigo-50/50 border-indigo-100"}`}
          >
            <div className="flex justify-between items-start mb-2 sm:mb-4">
              <p className="text-[8px] sm:text-[10px] font-black text-indigo-600 uppercase tracking-[0.2em]">
                Akumulasi Tabungan
              </p>
              <Percent className="text-indigo-600 opacity-30" size={16} />
            </div>
            <p className="text-lg sm:text-2xl font-black text-indigo-600 tracking-tighter truncate">
        ...
        <section
          id="tour-budgets-tracker"
          className={`p-5 sm:p-8 rounded-[2rem] sm:rounded-[3rem] border transition-all duration-500 shadow-xl relative overflow-hidden ${isDark ? "glass-dark border-white/5" : "glass border-white"}`}
        >
          <div className="flex items-center gap-2 sm:gap-3 mb-6 sm:mb-8 px-1">
            <div className="w-1 h-5 sm:w-1.5 h-6 bg-indigo-600 rounded-full" />
            <h2
