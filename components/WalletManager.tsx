...
import { X, Pencil, Trash2, Plus, CreditCard as CardIcon, Sparkles, RefreshCw } from 'lucide-react';
        ...
        <div className="p-6 space-y-6">
                    ...
                    >
                      <div className="flex justify-between items-start mb-3">
                        ...
                        <div className="flex gap-2">
                           <button
                             onClick={() => { setSyncingWallet(wallet); setTargetBalance(wallet.current_balance?.toString() || '0'); }}
          ...
          {syncingWallet && (
            <div className="fixed inset-0 bg-black/40 backdrop-blur-sm z-[250] flex items-center justify-center p-4">
              <div className={`w-full max-w-sm p-8 rounded-[2rem] border shadow-2xl animate-in zoom-in-95 duration-200 ${
                ...
                <div className="space-y-4 mb-8">
                  <div>
                    ...
                    <p className={`text-sm font-black ${isDark ? 'text-slate-300' : 'text-slate-600'}`}>
                      Rp {(syncingWallet.current_balance || 0).toLocaleString('id-ID')}
                    </p>
                  </div>
                  <div>
                    <label className={`block text-[10px] font-black uppercase tracking-widest mb-2 ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>
                    ...
                    <div className="relative">
                      <input 
                        ...
                        className={`w-full p-4 pl-12 border rounded-2xl outline-none transition-all text-lg font-black tracking-tighter ${
                          isDark ? 'bg-slate-800 border-white/10 text-white focus:border-blue-500/50' : 'bg-slate-50 border-slate-200 text-slate-900 focus:border-blue-500'
                        }`}
                        autoFocus
                      />
                      <span className="absolute left-5 top-1/2 -translate-y-1/2 text-xs font-black text-blue-500/50">RP</span>
                    </div>
                  </div>
                </div>
                ...
                <div className="flex gap-3">
                  ...
                  <button
                    ...
                    disabled={loading}
                    className="flex-1 py-4 bg-rose-600 text-white text-[10px] font-black uppercase tracking-widest rounded-xl hover:bg-rose-700 transition-all shadow-lg shadow-rose-500/20 disabled:opacity-50"
                  >
