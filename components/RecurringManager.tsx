...
export default function RecurringManager({ user, isDark, onClose }: { user: User, isDark: boolean, onClose: () => void }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (editingId) {
      const { error } = await supabase.from('recurring_templates').update(payload).eq('id', editingId);
  ...
  const executeManualTrigger = async (template: RecurringTemplate) => {
    ...
    try {
      if (template.type === 'transfer') {
        ...
        const fromWalletName = wallets.find(w => w.id === template.wallet_id)?.name;
        const toWalletName = wallets.find(w => w.id === template.target_wallet_id)?.name;
      ...
      await supabase.from('recurring_templates').update({ last_generated: new Date().toISOString() }).eq('id', template.id);
      fetchData();
  ...
  return (
    <div className="fixed inset-0 bg-black/60 backdrop-blur-md z-[100] flex items-center justify-center p-4">
            ...
            <div className="space-y-8">
              ...
              {loading && templates.length === 0 ? (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  {templates.map((t) => (
                    <div key={t.id} className={`p-6 rounded-[2rem] border transition-all hover:scale-[1.02] group relative overflow-hidden ${isDark ? 'bg-slate-800/40 border-white/5' : 'bg-slate-50 border-slate-200'}`}>
                          ...
                          <h4 className={`text-base font-black tracking-tight ${isDark ? 'text-white' : 'text-slate-900'}`}>
                            {t.type === 'transfer' ? (
                              <span className="flex items-center gap-2">
                                ...
                                <svg xmlns="http://www.w3.org/2000/svg" className="h-3 w-3 opacity-40" viewBox="0 0 20 20" fill="currentColor"><path fillRule="evenodd" d="M10.293 3.293a1 1 0 011.414 0l6 6a1 1 0 010 1.414l-6 6a1 1 0 01-1.414-1.414L14.586 11H3a1 1 0 110-2h11.586l-4.293-4.293a1 1 0 010-1.414z" clipRule="evenodd" /></svg>
                                {wallets.find(w => w.id === t.target_wallet_id)?.name}
                              </span>
                            ) : t.keterangan}
                          </h4>
                        </div>
                      ...
                      <div className={`mt-4 pt-4 border-t flex items-center justify-between ${isDark ? 'border-white/5' : 'border-slate-200'}`}>
                        ...
                        {t.last_generated && (
                          <p className="text-[8px] font-bold text-slate-500 uppercase tracking-wider">
                            Terakhir: {new Date(t.last_generated).toLocaleDateString('id-ID', { day: '2-digit', month: 'short' })}
                          </p>
                        )}
                      </div>
                    </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-8 animate-in slide-in-from-right-4 duration-500">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div className="space-y-6">
                  ...
                  <div>
                    <label className="block text-[10px] font-black uppercase tracking-widest mb-3 opacity-60">Value (IDR)</label>
                    <input type="number" value={formData.amount} onChange={(e) => setFormData({ ...formData, amount: Number(e.target.value) })} required className={`w-full px-5 py-4 rounded-2xl border outline-none transition-all ${isDark ? 'bg-slate-800 border-white/5 text-white' : 'bg-slate-50 border-slate-200'}`} />
                  </div>
                ...
                <div className="space-y-6">
                  <div className={`grid grid-cols-1 ${formData.type === 'transfer' ? 'md:grid-cols-2' : ''} gap-4`}>
                    <div>
                      <label className="block text-[10px] font-black uppercase tracking-widest mb-3 opacity-60">
                        {formData.type === 'transfer' ? 'Wallet Sumber' : 'Sumber/Tujuan Wallet'}
                      </label>
                    ...
                    {formData.type === 'transfer' && (
                      <div>
                        <label className="block text-[10px] font-black uppercase tracking-widest mb-3 opacity-60">Wallet Tujuan</label>
                        <select value={formData.target_wallet_id || ''} onChange={(e) => setFormData({ ...formData, target_wallet_id: e.target.value })} className={`w-full px-5 py-4 rounded-2xl border outline-none transition-all ${isDark ? 'bg-slate-800 border-white/5 text-white' : 'bg-slate-50 border-slate-200'}`} required={formData.type === 'transfer'}>
                          <option value="">Pilih Tujuan...</option>
                          {wallets.filter(w => w.id !== formData.wallet_id).map(w => <option key={w.id} value={w.id}>{w.icon} {w.name}</option>)}
                        </select>
                      </div>
                    )}
                  </div>
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      <label className="block text-[10px] font-black uppercase tracking-widest mb-3 opacity-60">Frekuensi</label>
                      <select value={formData.frequency} onChange={(e) => setFormData({ ...formData, frequency: e.target.value })} className={`w-full px-5 py-4 rounded-2xl border outline-none transition-all ${isDark ? 'bg-slate-800 border-white/5 text-white' : 'bg-slate-50 border-slate-200'}`}>
                        <option value="monthly">Bulanan</option>
                        <option value="weekly">Mingguan</option>
                      </select>
                    </div>
                    <div>
                      <label className="block text-[10px] font-black uppercase tracking-widest mb-3 opacity-60">Tgl Eksekusi (1-31)</label>
                      <input type="number" min="1" max="31" value={formData.day_of_month} onChange={(e) => setFormData({ ...formData, day_of_month: Number(e.target.value) })} className={`w-full px-5 py-4 rounded-2xl border outline-none transition-all ${isDark ? 'bg-slate-800 border-white/5 text-white' : 'bg-slate-50 border-slate-200'}`} />
                    </div>
                  </div>
                  <div>
                    <label className="block text-[10px] font-black uppercase tracking-widest mb-3 opacity-60">Kategori</label>
                    <select value={formData.kategori} onChange={(e) => setFormData({ ...formData, kategori: e.target.value })} className={`w-full px-5 py-4 rounded-2xl border outline-none transition-all ${isDark ? 'bg-slate-800 border-white/5 text-white' : 'bg-slate-50 border-slate-200'}`}>
                      {(formData.type === 'income' ? CATEGORIES.pemasukan : formData.type === 'saving' ? CATEGORIES.tabungan : formData.type === 'transfer' ? CATEGORIES.transfer : CATEGORIES.pengeluaran).map(c => (
      ...
      <ConfirmationModal 
        ...
        title={confirmingAction?.type === 'generate' ? "Generate Transaksi" : "Hapus Template"}
        message={confirmingAction?.type === 'generate' 
        ...
        confirmText={confirmingAction?.type === 'generate' ? "Generate 🪄" : "Hapus Permanen"}
        onConfirm={() => {
          if (confirmingAction) {
