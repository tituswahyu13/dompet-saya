...
export default function Insights({
  ...
  const categoryDataRaw = transactions
    ...
    .reduce((acc: any, curr) => {
      acc[curr.kategori] =
        (acc[curr.kategori] || 0) + Number(curr.outcome || 0);
      return acc;
    }, {});
  ...
  const categoryData = Object.keys(categoryDataRaw)
    .map((key) => {
  ...
  const dailyBalancesRaw = transactions.reduce((acc: any, curr) => {
    ...
    acc[date] +=
      Number(curr.income || 0) -
      Number(curr.outcome || 0) -
      Number(curr.saving || 0);
    return acc;
  }, {});

  const sortedDates = Object.keys(dailyBalancesRaw).sort();
  let runningBalance = 0;
  const balanceTrendData = sortedDates
    .map((date) => {
  ...
  return (
    <div className="space-y-10 mb-12">
      ...
      {transactions.length > 0 && (
        <div
          className={`p-6 rounded-[2.5rem] border flex items-center gap-5 transition-all duration-500 shadow-xl overflow-hidden relative ${
            isHealthy
              ? isDark
                ? "glass-dark border-green-500/20 shadow-green-900/10"
                : "glass border-green-200 shadow-green-200/50"
              : isDark
                ? "glass-dark border-red-500/20 shadow-red-900/10"
                : "glass border-red-200 shadow-red-200/50"
          }`}
        >
          {/* Animated Background Pulse */}
          <div
            className={`absolute -right-20 -top-20 w-48 h-48 rounded-full blur-[80px] opacity-20 animate-pulse ${isHealthy ? "bg-green-500" : "bg-red-500"}`}
          />

          <div
            className={`p-4 rounded-3xl flex-shrink-0 flex items-center justify-center shadow-lg ${
              isHealthy
          ...
          <div className="relative z-10">
            <p
              className={`text-[10px] font-black uppercase tracking-[0.2em] mb-1 ${isHealthy ? (isDark ? "text-green-400/70" : "text-green-600/70") : isDark ? "text-red-400/70" : "text-red-600/70"}`}
            >
              Wawasan Ekonomi
            </p>
            <p
              className={`text-md md:text-lg font-black tracking-tight ${isDark ? "text-white" : "text-slate-800"}`}
            >
              {healthMessage}
            </p>
            <p
              className={`text-[10px] font-bold ${isDark ? "text-slate-400" : "text-slate-500"} mt-1`}
            >
              {subMessage}
            </p>
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
        ...
        <div
          className={`p-10 rounded-[2.5rem] shadow-xl border flex flex-col h-[480px] transition-all duration-500 relative overflow-hidden group ${
            isDark
        ...
        <div
          className={`p-10 rounded-[2.5rem] shadow-xl border flex flex-col h-[400px] transition-all duration-500 relative overflow-hidden group lg:col-span-2 ${
            isDark
              ? "glass-dark border-white/5 shadow-black/20"
              : "glass border-white shadow-slate-200/50"
          }`}
