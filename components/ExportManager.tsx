...
export default function ExportManager({ transactions, wallets, isDark, onClose }: ExportManagerProps) {
  ...
  const getFilteredTransactions = () => {
    return transactions.filter(t => {
      ...
      const matchesDateRange = (!dateRange.start || t.tanggal >= dateRange.start) && 
  ...
  const exportToPDF = async () => {
    ...
    pdf.text(`Dompet Saya - Financial Report`, pageWidth / 2, 28, { align: 'center' });
    pdf.text(`Generated: ${format(new Date(), 'dd MMMM yyyy, HH:mm')}`, pageWidth / 2, 34, { align: 'center' });
    ...
    yPos += 6;
    pdf.text(`Pengeluaran: Rp ${totalOutcome.toLocaleString('id-ID')}`, 15, yPos);
    yPos += 6;
    ...
    filtered.slice(0, 30).forEach((t, index) => {
      ...
      pdf.text(t.income ? `Rp ${(t.income / 1000).toFixed(0)}k` : '-', 130, yPos);
      pdf.text(t.outcome ? `Rp ${(t.outcome / 1000).toFixed(0)}k` : '-', 160, yPos);
    ...
    if (filtered.length > 30) {
      ...
      pdf.text(`... dan ${filtered.length - 30} transaksi lainnya`, 15, yPos + 5);
    }
  ...
  return (
    <div className="fixed inset-0 bg-black/60 backdrop-blur-md z-[100] flex items-center justify-center p-4">
      <div className={`w-full max-w-2xl rounded-[2.5rem] border shadow-2xl animate-in zoom-in-95 duration-300 ${
        ...
        {/* Header */}
        <div className={`p-8 border-b ${isDark ? 'border-white/5' : 'border-slate-200'}`}>
