...
export default function TransferForm({ onRefresh, isDark, onCancel, user }: { 
  onRefresh: () => void, 
  isDark: boolean,
  onCancel: () => void,
  user: User
    ...
    <form onSubmit={handleSubmit} className={`p-8 rounded-[2.5rem] shadow-xl border transition-all duration-500 relative overflow-hidden ${
      ...
      <div className="space-y-6 relative z-10">
          ...
          <div>
            ...
            <div className="relative">
              <select 
                ...
                className={`w-full p-4 border rounded-2xl outline-none transition-all text-sm font-black appearance-none cursor-pointer ${
                  isDark ? 'bg-slate-950/50 border-white/5 text-white focus:border-green-500/50' : 'bg-white/50 border-slate-200 text-slate-900 focus:border-green-500'
                }`}
                required
              >
                {wallets.map(w => (
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <div>
            <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2 px-1">Catatan</label>
            <input 
              type="text"
              value={note}
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <div>
            <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2 px-1">Biaya Admin (Rp)</label>
            <div className="relative">
              <input 
                type="number"
                value={adminFee}
        ...
        {errorStatus && <p className="text-[10px] text-red-500 px-2 font-black uppercase tracking-widest">{errorStatus}</p>}
